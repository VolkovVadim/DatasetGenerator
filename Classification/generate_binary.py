...
def generate(func_type: int = 1) -> pd.DataFrame:
    ...
    if func_type == 1:
        Y = [1 if F1[i] - F2[i] > 0 else 0 for i in range(EXAMPLES_COUNT)]

    if func_type == 2:
        Y = [1 if F1[i] - 10.0 * np.sin(0.25 * F2[i]) > 0 else 0 for i in range(EXAMPLES_COUNT)]

    if func_type == 3:
        Y = [1 if F1[i] - (F2[i] + 5.5 * np.sin(0.5 * F2[i])) > 0 else 0 for i in range(EXAMPLES_COUNT)]
...
if __name__ == "__main__":
    ...
    # Save data to file
    if args.save:
        dataset_filename = f"binary_classification_v{FUNC_TYPE}_{EXAMPLES_COUNT}.csv"
